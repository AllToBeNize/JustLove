...
FWidgetTreeSerializer::FWidgetEntry FWidgetTreeSerializer::ParseEntryLine(const FString& Line, int32 Depth)
{
	...
	if (JsonObj->HasField(TEXT("Slot")))
	{
		...
		if (JsonObj->TryGetObjectField(TEXT("Slot"), SlotObj))
		{
			...
			{
				// UE 5.8 stores JSON object keys as FSharedString. Convert the key
				// explicitly so this code also works with APIs that still require FString.
				const FString Key(Pair.Key.ToView());
				if (Pair.Value->Type == EJson::String)
				{
					Entry.SlotProperties.Add(Key, Pair.Value->AsString());
				}
				else
				{
					FString ValueStr;
					...
					FJsonSerializer::Serialize(Pair.Value, Key, Writer);
					Entry.SlotProperties.Add(Key, MoveTemp(ValueStr));
				}
			}
		}
	}
	...
	{
		const FString Key(Pair.Key.ToView());
		if (Key == TEXT("Type") || Key == TEXT("bIsVariable") || Key == TEXT("Slot"))
		{
			continue;
		}
		...
		if (Pair.Value->Type == EJson::String)
		{
			Entry.Properties.Add(Key, Pair.Value->AsString());
		}
		else
		{
			FString ValueStr;
			TSharedRef<TJsonWriter<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>> Writer =
				TJsonWriterFactory<TCHAR, TCondensedJsonPrintPolicy<TCHAR>>::Create(&ValueStr);
			FJsonSerializer::Serialize(Pair.Value, Key, Writer);
			Entry.Properties.Add(Key, MoveTemp(ValueStr));
		}
	}
